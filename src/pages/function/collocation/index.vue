    ...
    <div class="panel-body">
      ...
      <div v-show="Edition == '一键搭配'">
        <el-form
          ...
          label-width="80px"
        >
          <el-form-item label="搭配名称">
            <el-input
              v-model="form.collocaName"
              placeholder="请输入搭配名称"
            ></el-input>
          </el-form-item>
...
export default {
  ...
  methods: {
    get_table_data_filter() {
      var url = "";
      if (this.Edition == "一键搭配") {
        url = "/collocat/selectCollocat";
      } else if (this.Edition == "在线版本") {
        url = "/collocat/selectListEdiCloth";
      } else {
        url = "/collocat/selectListEdiOutCloth";
      }
      this.load_data = true;
      var form = this.form;
      ...
      this.$confirm("是否确定删除?", "提示", {
        ...
        .then(() => {
          ...
          if (this.Edition == "一键搭配") {
            ...
            url = "/collocat/delEditionColloca";
            data1 = { editionId: data.editionId };
          } else {
            url = "/collocat/delOutEditionColloca";
            data1 = { editionId: data.editionId };
          }
          this.$http({
            url: url,
      ...
      this.$confirm("是否确定发布该版本?", "提示", {
        ...
        .then(() => {
          ...
          if (this.Edition == "在线版本") {
            url = "/collocat/release/online";
            data1 = { editionId: data.editionId };
          } else {
            url = "/collocat/release/offline";
            data1 = { editionId: data.editionId };
          }
          this.$http({
            url: url,
    ...
    //新增
    addForm() {
      this.$router.push({ path: "/collocationAdd" });
    },
    // 新增成功回调
    dialogChange(val) {
      this.dialogMsg = val;
